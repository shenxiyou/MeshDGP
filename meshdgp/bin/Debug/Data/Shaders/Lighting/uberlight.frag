...
float distanceShape(vec3 pos)
{
   ...
   return smoothstep(DsNearEdge, DsNear, depth) * (1.0 - smoothstep(DsFar, DsFarEdge, depth));
}

void main()
{
    float attenuation = superEllipseShape(LCpos) * distanceShape(LCpos);
   // gl_FragColor = vec4(computeShadow() * gl_Color.rgb * attenuation, gl_Color.a);
    
     gl_FragColor = vec4( gl_Color.rgb * attenuation, gl_Color.a);
}
