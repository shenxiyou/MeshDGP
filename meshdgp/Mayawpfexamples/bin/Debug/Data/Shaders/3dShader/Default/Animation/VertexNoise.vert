...
void main(void)
{
	...
	//vec3 vertex = gl_Vertex.xyz +   (offset + gl_Vertex.xyz * scaleIn)*snoise(offset + gl_Vertex.xyz * scaleIn) * scaleOut;

    vec3 temp=offset + gl_Vertex.xyz * scaleIn;
    vec3 result=vec3(snoise(temp + vec3(0.0, 0.0, 0)),snoise(temp + vec3(43.0, 17.0, 0)),snoise(temp + vec3(-17.0, -43.0, 0)));
    vec3 vertex = gl_Vertex.xyz + result * scaleOut;
